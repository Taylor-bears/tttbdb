...
Rid RmFileHandle::insert_record(char* buf, Context* context) {
    (void)context;
    if (buf == nullptr) throw InternalError("RmFileHandle::insert_record null data");
    RmPageHandle page_handle = create_page_handle();
    int slot_no = Bitmap::first_bit(false, page_handle.bitmap, file_hdr_.num_records_per_page);
    if (slot_no == file_hdr_.num_records_per_page) {
        buffer_pool_manager_->unpin_page(page_handle.page->get_page_id(), false);
        throw InternalError("Free record page is full");
    }
    ...
    if (page_handle.page_hdr->num_records == file_hdr_.num_records_per_page) {
        if (file_hdr_.first_free_page_no == rid.page_no) {
            ...
            while (current != RM_NO_PAGE) {
                RmPageHandle node = fetch_page_handle(current);
                int next = node.page_hdr->next_free_page_no;
                if (next == rid.page_no) {
                    node.page_hdr->next_free_page_no = page_handle.page_hdr->next_free_page_no;
                    buffer_pool_manager_->unpin_page(node.page->get_page_id(), true);
                    break;
                }
                buffer_pool_manager_->unpin_page(node.page->get_page_id(), false);
                current = next;
            }
        }
        page_handle.page_hdr->next_free_page_no = RM_NO_PAGE;
...
void RmFileHandle::update_record(const Rid& rid, char* buf, Context* context) {
    (void)context;
    if (buf == nullptr) throw InternalError("RmFileHandle::update_record null data");
    RmPageHandle page_handle = fetch_page_handle(rid.page_no);
    if (rid.slot_no < 0 || rid.slot_no >= file_hdr_.num_records_per_page ||
        !Bitmap::is_set(page_handle.bitmap, rid.slot_no)) {
        buffer_pool_manager_->unpin_page(page_handle.page->get_page_id(), false);
...
RmPageHandle RmFileHandle::create_new_page_handle() {
    ...
    if (page_id.page_no != file_hdr_.num_pages) {
        ...
        throw InternalError("Unexpected record page number");
    }
    ++file_hdr_.num_pages;
