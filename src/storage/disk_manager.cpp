...
void DiskManager::write_page(int fd, page_id_t page_no, const char *offset, int num_bytes) {
    ...
    if (lseek(fd, file_offset, SEEK_SET) == static_cast<off_t>(-1)) {
        throw UnixError();
    }
    int written = 0;
    while (written < num_bytes) {
        ssize_t n = write(fd, offset + written, num_bytes - written);
        if (n <= 0) {
            throw InternalError("DiskManager::write_page Error");
        }
...
void DiskManager::read_page(int fd, page_id_t page_no, char *offset, int num_bytes) {
    ...
    while (bytes_read < num_bytes) {
        ssize_t n = read(fd, offset + bytes_read, num_bytes - bytes_read);
        if (n <= 0) {
            throw InternalError("DiskManager::read_page Error");
        }
...
void DiskManager::create_file(const std::string &path) {
    ...
    int fd = open(path.c_str(), O_CREAT | O_EXCL | O_RDWR | O_BINARY, S_IRUSR | S_IWUSR);
    if (fd < 0) {
...
int DiskManager::open_file(const std::string &path) {
    ...
    if (fd >= MAX_FD) {
        close(fd);
        throw InternalError("Too many open files");
    }
    path2fd_[path] = fd;
    fd2path_[fd] = path;
    return fd;
